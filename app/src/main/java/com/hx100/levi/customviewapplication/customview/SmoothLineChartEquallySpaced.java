...
//			mPaint.getTextBounds(String.valueOf(mValues[i]), 0, String.valueOf(mValues[i]).length(), textBox);
...
//			float textY = changeUnit(17, "sp");
			...
			for (int i = 0; i < points.size(); i++) {
				...
				float textX= (float) textBox.width();
				float textY = changeUnit(17, "sp");
				if (changeUnit((int) (mMaxY - points.get(i).y), "sp") > textY) {
					canvas.drawText(String.valueOf(String.valueOf(mValues[i])), points.get(i).x - (textX / 2), points.get(i).y - textY, mPaint);
				} else {
					canvas.drawText(String.valueOf(String.valueOf(mValues[i])), points.get(i).x - (textX / 2), points.get(i).y + textY, mPaint);
				}
			}
