...
public class HeaderScrollingBehavior extends CoordinatorLayout.Behavior<RecyclerView> {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, RecyclerView child, int layoutDirection) {
        ...
        if (lp.height == CoordinatorLayout.LayoutParams.MATCH_PARENT) {
            LogUtil.i("HeaderScrollingBehavior.onLayoutChild    if inerr");
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, RecyclerView child, View dependency) {
        ...
        LogUtil.i("HeaderScrollingBehavior.onDependentViewChanged-----------progress=="+progress);
        child.setTranslationY(dependency.getHeight() + dependency.getTranslationY());
        LogUtil.i("HeaderScrollingBehavior.onDependentViewChanged-----------dependency.getTranslationY()=="+dependency.getTranslationY());

        float scale = 1 + 0.4f * (1.f - progress);
        LogUtil.i("HeaderScrollingBehavior.onDependentViewChanged-----------scale=="+scale);
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, RecyclerView child, View target, int dx, int dy, int[] consumed) {
        LogUtil.i("HeaderScrollingBehavior.onNestedPreScroll-----------dy=="+dy);
        if (dy < 0) {
            return;
        }
        ...
        LogUtil.i("HeaderScrollingBehavior.onNestedPreScroll-----------newTranslateY=="+newTranslateY);
        LogUtil.i("HeaderScrollingBehavior.onNestedPreScroll-----------minHeaderTranslate=="+minHeaderTranslate);
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, RecyclerView child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        LogUtil.i("HeaderScrollingBehavior.onNestedScroll-----------dyConsumed=="+dyConsumed);
        LogUtil.i("HeaderScrollingBehavior.onNestedScroll-----------dyUnconsumed=="+dyUnconsumed);
        if (dyUnconsumed > 0) {
            return;
        }
